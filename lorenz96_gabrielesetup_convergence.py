...
CLV = np.zeros((len(t),dimN,M,len(hs)),dtype=np.float64)
BLV = np.zeros((len(t),dimN,M,len(hs)),dtype=np.float64)
R = np.zeros((len(t),dimN,M,len(hs)),dtype=np.float64)
lyapmean_blv = np.zeros((M,len(hs)),dtype=np.float64)
lyapmean_clv = np.zeros((M,len(hs)),dtype=np.float64)
lyaploc_clv = np.zeros((len(t),M,len(hs)),dtype=np.float64)
lyaploc_blv = np.zeros((len(t)-1,M,len(hs)),dtype=np.float64)



# Compute the exponents
savename='gabrielessetup_convergence_first_run_v2'
...
CLV2 = np.zeros((len(t),dimN,M,len(hs)),dtype=np.float64)
BLV2 = np.zeros((len(t),dimN,M,len(hs)),dtype=np.float64)
R2 = np.zeros((len(t),dimN,M,len(hs)),dtype=np.float64)
lyapmean_blv2 = np.zeros((M,len(hs)),dtype=np.float64)
lyapmean_clv2 = np.zeros((M,len(hs)),dtype=np.float64)
lyaploc_clv2 = np.zeros((len(t),M,len(hs)),dtype=np.float64)
lyaploc_blv2 = np.zeros((len(t)-1,M,len(hs)),dtype=np.float64)
...
savename='gabrielessetup_convergence_second_run_v2'
if not os.path.exists(savename): os.mkdir(savename)


np.save(savename+'/lyaploc_blv2',lyaploc_blv2)
np.save(savename+'/lyaploc_clv2',lyaploc_clv2)
np.save(savename+'/lyapmean_blv2',lyapmean_blv2)
np.save(savename+'/lyapmean_clv2',lyapmean_clv2)
np.save(savename+'/CLV2',CLV2)
np.save(savename+'/BLV2',BLV2)
np.save(savename+'/R2',R2)
